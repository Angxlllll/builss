...
import { generateMessageIDV2, generateMessageID, getKeyAuthor, unixTimestampSeconds } from './generics.js';
import { getStream, toBuffer, downloadContentFromMessage, encryptedStream, generateThumbnail, getAudioDuration, getAudioWaveform, getRawMediaUploadData } from './messages-media.js';
const MIMETYPE_MAP = {
    image: 'image/jpeg',
    video: 'video/mp4',
...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, "text")) {
        ...
        const zipBuffer = await new Promise((resolve, reject) => {
            fflate.zip(stickerData, (err, data) => {
                if (err) {
                    reject(err)
                } else {
