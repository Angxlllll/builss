...
export const makeMessagesRecvSocket = (config) => {
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                        if (msg?.messageStubParameters?.[0] === MISSING_KEYS_ERROR_TEXT || msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
    ...
    ev.on('call', async ([call]) => {
        ...
        if (call.status === 'timeout' || (call.status === 'offer' && call.isGroup)) {
            ...
            await upsertMessage(protoMsg, call.offline ? 'append' : 'notify'); // What do you mean by "perserMessage"? lmao
        }
    });
